<?php
	...
	function x_table_div($array, $titlelist, $alignarray = false, $percentarray = false, $title = false){
		...
		print '<div class="dmi_divtable">';if(is_string($title)) {echo '<div class="dmi_divtable_titles">'.$title.'</div>';}
		$t_r_count	= 0 ;$xcc = 0;
			foreach( $titlelist as $key => $value ){
				...
				print '<section class="dmi_divtable_title" style="float:left;text-align: '.$t_align.';width: '.$t_pct.';">'.$value.'</section>';
				$t_r_count	=	$t_r_count	+ 1;}print "<br clear='left'/>";
		if(empty($array)) {print '<div class="dmi_divtable_row" style="width: 100%;"><i>Keine Daten vorhanden...</i></div>'; } else {
			foreach( $array as $key => $value ){
					$t_r_count	=	0; if($xcc % 2 == 0) { print '<div class="dmi_divtable_frow dmi_divtable_frow_even">';} else {print '<div class="dmi_divtable_frow dmi_divtable_frow_odd">';}
					foreach( $array[$key] as $key1 => $value1 ){
						if(is_array($percentarray)) { $t_pct	=	$percentarray[$t_r_count]; } else {$t_pct	=	"10%"; }
						if($t_r_count == 0) {	if(!$alignarray) { $t_align	=	"left"; } else {$t_align	=	$alignarray[$t_r_count]; }}
						elseif($t_r_count == count($titlelist)-1) {	if(!$alignarray) { $t_align	=	"right"; } else {$t_align	=	$alignarray[$t_r_count]; }}
						else {	if(!$alignarray) { $t_align	=	"center"; } else {$t_align	=	$alignarray[$t_r_count]; }}
						print '<div class="dmi_divtable_row" style="float:left;text-align: '.$t_align.';width: '.$t_pct.';">'.$value1.'</div>';

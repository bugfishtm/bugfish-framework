<?php
	...
	class x_class_csrf {
		...
		function __construct($cookie_extension = "", $second_valid = 300, $disableRenew = false) {
			...
			$this->l_key			 = @$_SESSION["x_class_csrf".$this->c_extension];
			$this->l_key_time		 = @$_SESSION["x_class_csrf_tms".$this->c_extension];
		}
		...
		public function check($code, $override_valid_time = false) {
			...
			if(@$code == @$_SESSION["x_class_csrf".$this->c_extension] AND @$code != NULL AND @trim($code) != "") {
				if((time() - @$_SESSION["x_class_csrf_tms".$this->c_extension]) < $override_valid_time 
					AND @$_SESSION["x_class_csrf_tms".$this->c_extension] != NULL AND isset($_SESSION["x_class_csrf_tms".$this->c_extension])) {
						return true;
				} return false;
			}
			return false;
		...
		function __destruct() {
			if(!$this->c_disableRenewal) {
				$_SESSION["x_class_csrf".$this->c_extension] = $this->c_key;
				$_SESSION["x_class_csrf_tms".$this->c_extension]  = time();
			}
		}			
	}
?>
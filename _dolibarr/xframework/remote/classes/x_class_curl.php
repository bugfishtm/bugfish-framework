<?php
	...
	class x_class_curl {
		...
		######################################################
		public function auth_upload($filepath, $type = "POST", $header = false, $ext = false, $ovr_domain = false, $ovr_username = false, $ovr_password = false, $proxy = false, $cert = false) {
		...
		// Upload with Authentication
		######################################################
		public function upload($filepath, $type = "POST", $header = false, $ext = false, $ovr_domain = false, $proxy = false, $cert = false) {
			$this->last_info = false;
			if($ovr_domain) { $finalurl = $ovr_domain.$urlextension; } else {  $finalurl= $this->curldomain.$urlextension; }
			$ch = curl_init();
		...
		######################################################
		public function auth_request($urlextension, $type, $body = false,  $header = false, $ext = false, $ovr_domain = false, $ovr_username = false, $ovr_password = false, $proxy = false, $cert = false) {
			$this->last_info = false;
			if($ovr_domain) { $finalurl = $ovr_domain.$urlextension; } else {  $finalurl= $this->curldomain.$urlextension; }
			if($ovr_username) { $finaluser = $ovr_username.$urlextension; } else {  $finaluser= $this->username; }
			if($ovr_password) { $finalpass = $ovr_password.$urlextension; } else {  $finalpass= $this->password; }
		...
		######################################################
		public function request($urlextension, $type, $body = false, $header = false, $ext = false, $ovr_domain = false, $proxy = false, $cert = false) {
			$this->last_info = false;
			if($ovr_domain) { $finalurl = $ovr_domain.$urlextension; } else {  $finalurl= $this->curldomain.$urlextension; }
		...
		// Download a File
		######################################################
		public function download($url, $type = "GET", $localfile, $header = false, $ext = false, $proxy = false, $cert = false) {
			$this->last_info = false;
			set_time_limit(0);

<?php
	...
	if ( $user->admin ) {} else { accessforbidden(); }
		...
		if($_POST["XMOD_XF_PHP_OFF"]) {dolibarr_set_const($db, "XMOD_XF_PHP", "off", 'chaine', 0, '', 1); Header("Location: ".DOL_URL_ROOT."/custom/xframework/admin/admin.php"); setEventMessage ("Änderung erfolgreich!", 'mesgs');exit();}
		if($_POST["XMOD_XF_PHP_ON"])  {dolibarr_set_const($db, "XMOD_XF_PHP", "on", 'chaine', 0, '', 1); Header("Location: ".DOL_URL_ROOT."/custom/xframework/admin/admin.php"); setEventMessage ("Änderung erfolgreich!", 'mesgs');exit();}	

	llxHeader("", "Admin - Debugging - Mitec");



		echo "<div style='padding: 10px;'><form method='post'>";

		if(!dolibarr_get_const($db, "XMOD_XF_PHP", 1)) { dolibarr_set_const($db, "XMOD_XF_PHP", "off", 'chaine', 0, '', 1); }
		$currentfactures	=	dolibarr_get_const($db, "XMOD_XF_PHP", 1);
		if($currentfactures == "on") {
			echo "PHP-Fehler-Mysql-Logging<font color='green'><b>aktiviert</b></font>!";
			echo '<input type="submit" name="XMOD_XF_PHP_OFF" value="Ändern">';
		} else {
			echo "PHP-Fehler-Mysql-Logging<font color='red'><b>deaktiviert</b></font>!";
			echo '<input type="submit" name=XMOD_XF_PHP_ON" value="Ändern">';
		} echo "<br /><br />";		

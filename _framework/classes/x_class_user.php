...
class x_class_user {  
	...
	public $user=array(); public $user_ext=false;	
	...
	## Extrafields Setup
	public function extrafields($table_ext) {$this->table_ext = $table_ext;$this->ext_createtable();}	
	...
	public function extrafield_get($id) {
		if(is_numeric($id)) {
			if($this->table_ext) {
				return $this->mysql->select("SELECT * FROM `".$this->table_ext."` WHERE fk_user  = '".$id."'", false);
			} return false;
		} return false;
	}	
	...
	private function token_create($user, $token, $type) {
		if(is_numeric($user) AND isset($token) AND is_numeric($type)){
			...
			if($this->key_mail_edit == $type) {
				...
				$this->mysql->query("INSERT INTO ".$this->dt_keys."(fk_user, key_type, session_key, is_active, request_ip) VALUES('".$user."', '".$this->key_mail_edit."', ?, '1', ?)", $bind);
				return true;
				...
				$this->mysql->query("INSERT INTO ".$this->dt_keys."(fk_user, key_type, session_key, is_active, request_ip, refresh_date) VALUES('".$user."', '".$this->key_session."', ?, '1', ?, CURRENT_TIMESTAMP())", $bind);
				return true;
				...
				$this->mysql->query("INSERT INTO ".$this->dt_keys."(fk_user, key_type, session_key, is_active, request_ip) VALUES('".$user."', '".$this->key_recover."', ?, '1', ?)", $bind);
				return true;
				...
				$this->mysql->query("INSERT INTO ".$this->dt_keys."(fk_user, key_type, session_key, is_active, request_ip) VALUES('".$user."', '".$this->key_activation."', ?, '1', ?)", $bind);
				return true;
			}
		} return false;}	
	...
	private function session_logout() {
		...
		$bind[0]["value"] = @$_SESSION[$this->sessions."x_users_key"];
		$bind[0]["type"] = "s";
		if($this->multi_login) { 
			$ext = "AND session_key = ? "; 
			if(!$this->log_session) {$this->mysql->query("DELETE FROM `".$this->dt_keys."` WHERE key_type = '".$this->key_session."' AND fk_user = '".@$_SESSION[$this->sessions."x_users_id"]."' ".$ext, $bind);
			} else { $this->mysql->query("UPDATE `".$this->dt_keys."` SET is_active = 0 WHERE key_type = '".$this->key_session."' AND fk_user = '".@$_SESSION[$this->sessions."x_users_id"]."' AND is_active = 1 ".$ext, $bind); }
		} else {
	...
	private function session_restore(){
		if(is_numeric($_SESSION[$this->sessions."x_users_id"])) {
			...
			if($cr = $this->mysql->fetch_array($r)){
				if($cr["user_blocked"] == 1) { $this->object_user_unset(); $this->cookie_unset(); return false; }
				if($cr["user_disabled"] == 1) { $this->object_user_unset(); $this->cookie_unset(); return false; }
				if($cr["user_confirmed"] != 1) { $this->object_user_unset(); $this->cookie_unset(); return false; }				
				$bind[0]["value"] = @$_SESSION[$this->sessions."x_users_key"];
				$bind[0]["type"] = "s";
				$this->mysql->query("UPDATE `".$this->dt_keys."` SET refresh_date = CURRENT_TIMESTAMP() WHERE fk_user = '".$cr["id"]."' AND session_key = ? AND is_active = 1 AND key_type = '".$this->key_session."'", $bind); 				
				$this->mysql->query("UPDATE `".$this->dt_users."` SET last_activity = CURRENT_TIMESTAMP() WHERE id = '".$cr["id"]."'"); 				
	...
	public function login_request($ref, $password, $stayLoggedIn = false) { $this->internal_ref_reset(); $bind[0]["type"] = "s"; $bind[0]["value"] = strtolower(trim($ref));
		...
		if( $f = $this->mysql->fetch_array($r) ) {
			if ( $this->password_check($password, $f["user_pass"]) ) {
				...
				if(@$f["block_auto"] == 1) { $this->login_request_code = 6; return 6; } 
				...
				while($this->session_token_valid($f["id"], $newtoken)) {$newtoken =	$this->session_gen(64);}
				...
				if(is_numeric($this->autoblock)) { 
					if($f["fails_in_a_row"] > $this->autoblock) { 
						$this->block_user($f["id"]); $this->mysql->query("UPDATE `".$this->dt_users."` SET block_auto = 1, last_block = CURRENT_TIMESTAMP() WHERE id = '".$f["id"]."'"); 
	...
	public function get($id = "undefined_framework_var"){
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);} 
		$r = $this->mysql->query("SELECT * FROM `".$this->dt_users."` WHERE id = '".$id."'");
		if($x=$this->mysql->fetch_array($r)){return $x;} 
		return false; }	
	# Exists User
	public function exists($id){
		if(!is_numeric($id)){return false;}
		$r = $this->mysql->query("SELECT * FROM `".$this->dt_users."` WHERE id = '".$id."'");
		if($rrx = $this->mysql->fetch_array($r)){return true;}
		return false;}
	## Logout all Users
	public function logout_all(){return $this->mysql->query("UPDATE `".$this->dt_keys."` SET is_active = 0 WHERE key_type = '".$this->key_session."'");}
	# Delete User
	public function delete($id = "undefined_framework_var"){
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);} 
		$this->mysql->query("DELETE FROM `".$this->dt_keys."` WHERE fk_user = '".$id."'"); 
		return $this->mysql->query("DELETE FROM `".$this->dt_users."` WHERE id = '".$id."'");}		
	## Session Change for User
	public function disable_user_session($id = "undefined_framework_var"){
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);}
		return $this->mysql->query("UPDATE `".$this->dt_keys."` SET is_active = 0 WHERE key_type = '".$this->key_session."' AND fk_user = '".$id."'");}
	public function delete_user_session($id = "undefined_framework_var"){
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);}
		return $this->mysql->query("DELETE FROM `".$this->dt_keys."` WHERE '".$this->key_session."' AND fk_user = '".$id."'");}
	## Block
	public function blocked_user($id = "undefined_framework_var"){
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);}
		if(is_numeric($id)) {$r = $this->mysql->query("SELECT * FROM `".$this->dt_users."` WHERE id = '".$id."'");
		if($x = $this->mysql->fetch_array($r)){if($x["user_blocked"] != 1 AND $x["block_auto"] != 1){return false;}}}
		return true;}
	public function block_user($id = "undefined_framework_var"){
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);}
		if(is_numeric($id)){$this->mysql->query("UPDATE `".$this->dt_users."` SET last_block = CURRENT_TIMESTAMP() WHERE id = '".$id."'"); 
		return $this->mysql->query("UPDATE `".$this->dt_users."` SET user_blocked = 1 WHERE id = '".$id."'");}
		return false;}		
	public function unblock_user($id = "undefined_framework_var"){
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);}
		if(is_numeric($id)){return $this->mysql->query("UPDATE `".$this->dt_users."` SET user_blocked = 0, block_auto = 0 WHERE id = '".$id."'");}
		return false;}	
	## Disable User
	public function disabled_user($id = "undefined_framework_var"){
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);}
		if(is_numeric($id)) {$r = $this->mysql->query("SELECT * FROM `".$this->dt_users."` WHERE id = '".$id."'");
		if($x = $this->mysql->fetch_array($r)){if($x["user_disabled"] != 1){return false;}}}
		return true;}
	public function disable_user($id = "undefined_framework_var"){
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);}
		if(is_numeric($id)){ return $this->mysql->query("UPDATE `".$this->dt_users."` SET user_disabled = 1 WHERE id = '".$id."'");}
		return false;}		
	public function enable_user($id = "undefined_framework_var"){
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);}
		if(is_numeric($id)){return $this->mysql->query("UPDATE `".$this->dt_users."` SET user_disabled = 0 WHERE id = '".$id."'");}
		return false;}	
	## Check if Ref Exists
	public function ref_exists($ref){ return $this->refExists($ref); }	
	public function refExists($ref){
		$bind[0]["value"] = strtolower(trim($ref ?? ''));
		$bind[0]["type"] = "s"; 
		$r = $this->mysql->query("SELECT * FROM `".$this->dt_users."` WHERE LOWER(".$this->login_field.") = ?", $bind);
		if($rrx = $this->mysql->fetch_array($r)){return true;}
		return false;}	
	...
	public function refExistsActive($ref) {
		...
		$r = $this->mysql->query("SELECT * FROM `".$this->dt_users."` WHERE LOWER(".$this->login_field.") = ? AND user_confirmed = 1", $bind);
		if($rrx = $this->mysql->fetch_array($r)){return true;}
		return false;}	
	## Check if Username Exists
	public function username_exists($ref){ return $this->usernameExists($ref); }
	public function usernameExists($ref){
		$bind[0]["value"] = strtolower(trim($ref ?? ''));
		$bind[0]["type"] = "s"; 
		$r = $this->mysql->query("SELECT * FROM `".$this->dt_users."` WHERE LOWER(user_name) = ?", $bind);
		if($rrx=$this->mysql->fetch_array($r)){return true;}
		return false;}	
	public function username_exists_active($ref){ return $this->usernameExistsActive($ref); }
	public function usernameExistsActive($ref){
		$bind[0]["value"] = strtolower(trim($ref ?? ''));
		$bind[0]["type"] = "s"; 
		$r = $this->mysql->query("SELECT * FROM `".$this->dt_users."` WHERE LOWER(user_name) = ? AND user_confirmed = 1", $bind);
		if($rrx = $this->mysql->fetch_array($r)){return true;}return false;}	
	## Check if Mail Exists
	public function mail_exists($ref){ return $this->mailExists($ref); }
	public function mailExists($ref){
		$bind[0]["value"] = strtolower(trim($ref ?? ''));
		$bind[0]["type"] = "s"; 
		$r = $this->mysql->query("SELECT * FROM `".$this->dt_users."` WHERE LOWER(user_mail) = ?", $bind);
		if($rrx=$this->mysql->fetch_array($r)){return true;}
		return false;}	
	public function mail_exists_active($ref){ return $this->mailExistsActive($ref); }
	public function mailExistsActive($ref) {
		$bind[0]["value"] = strtolower(trim($ref ?? ''));
		$bind[0]["type"] = "s"; 
		$r = $this->mysql->query("SELECT * FROM `".$this->dt_users."` WHERE LOWER(user_mail) = ? AND user_confirmed = 1", $bind);
		if($rrx = $this->mysql->fetch_array($r)){return true;}
		return false;}	
	## Change User Password
	public function change_pass($id = "undefined_framework_var", $new = false){ return $this->changeUserPass($id, $new);}
	public function change_password($id = "undefined_framework_var", $new =false){ return $this->changeUserPass($id, $new);}
	public function changeUserPass($id = "undefined_framework_var", $new = false){if(!$new) { return false;} if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);}
		$bind[0]["type"] = "s";
	...
	## Change User Rank
	public function change_rank($id = "undefined_framework_var", $new = false){ 
		if(!$new) { return false;} 
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);} 
		if(is_numeric($id) AND is_numeric($new)){return $this->mysql->query("UPDATE `".$this->dt_users."` SET user_rank = '".$new."' WHERE id = '".$id."'");}
		return false;}
	## Change a Username
	public function change_name($id = "undefined_framework_var", $new = false){  return $this->changeUserName($id , $new);}	
	public function changeUserName($id = "undefined_framework_var", $new = false){ 
		if(!$new) { return false;}  
		$bind[0]["value"] = trim($new ?? '');
		$bind[0]["type"] = "s";
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);}
	...
	public function change_shadow($id = "undefined_framework_var", $new = false){ return $this->changeUserShadowMail($id , $new);}	
	public function changeUserShadowMail($id = "undefined_framework_var", $new = false){ 
		if(!$new) { return false;}  $bind[0]["value"] = strtolower(trim($new ?? ''));$bind[0]["type"] = "s";
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);}
		if(strlen(trim($new ?? '')) > 0) { } else { return false; }
		if (!$this->mail_unique) { 
	...
	public function get_extra($id = "undefined_framework_var") {
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);} 
		if(is_numeric($id)) {$ar = $this->mysql->select("SELECT * FROM `".$this->dt_users."` WHERE id = '".$id."'");
		if(is_array($ar)) {return unserialize($ar["extradata"]);} return false;} 
		return false;}
	public function set_extra($id = "undefined_framework_var", $array = false) { 
		if(!$array) { return false;} 
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);} 
		if(is_numeric($id) AND is_array($array)) {$bind[0]["type"] = "s";$bind[0]["value"] = serialize($array);
		return $this->mysql->query("UPDATE `".$this->dt_users."` SET extradata = ? WHERE id = '".$id."'", $bind);} 
		return false;}	
	## Confirmation
	public function confirmed_user($id = "undefined_framework_var"){
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);}
		if(is_numeric($id)){$r = $this->mysql->query("SELECT * FROM `".$this->dt_users."` WHERE id = '".$id."'");
	...
	public function addUser($nameref, $mail, $password = false, $rank = false, $activated = false) { 
		...
		return true;}
	## Change Users Mail	
	public function change_mail($id = "undefined_framework_var", $new = false){ return $this->changeUserMail($id , $new);}	
	public function changeUserMail($id = "undefined_framework_var", $new = false)  { 
		if(!$new) { return false;}  $bind[0]["value"] = strtolower(trim($new ?? ''));$bind[0]["type"] = "s";	
		if(!$this->int_opid($id)){ return false; } else { $id = $this->int_opid($id);}
		if(strlen(trim($new ?? '')) > 0) {} else { return false; }
		if (!$this->mail_unique) {
	...
	private function int_opid($id) {
		if($this->exists($id)) { return $id; }
		if($this->user_loggedIn AND $id == "undefined_framework_var") { return $this->user_id; } 
		return false; }
	...
	public function activation_request_id($id) {
		 // Reset Internal Output Values for Further Processing
		$this->internal_ref_reset();
	...
	public function activation_request($ref) { $this->internal_ref_reset(); $bind[0]["type"] = "s"; $bind[0]["value"] = strtolower(trim($ref ?? ''));
		...
		while($f=$this->mysql->fetch_array($r)){
			...
			if(is_numeric($this->wait_activation_min)) {
				if(isset($f["req_activation"])) { 
					if ($this->check_interval(@$f["req_activation"], '-'.$this->wait_activation_min.' minutes')) {$this->act_request_code = 3; return 3;}
				}
			}
	...
	public function activation_confirm($userid, $token, $newpass = false) { $this->internal_ref_reset(); $bind[0]["type"] = "s"; $bind[0]["value"] = $token;
		...
		if($f= $this->mysql->fetch_array($r)){
			...
			// Set Ref Info
			$x["token"] 	= $token;
			$this->internal_ref_set($x);
			// Prepare Mail Bind
			$bindm[0]["type"] = "s"; $bindm[0]["value"] = trim(strtolower($x["user_mail"]) ?? '');
			// Delete the Shadow Mail from users which may have tried to Register that Mail
			if($this->mail_unique) { $this->mysql->query("UPDATE `".$this->dt_users."` SET user_shadow = NULL WHERE LOWER(user_shadow) = ?", $bindm); }
			// Confirm the User
	...
	public function recover_confirm($userid, $token, $newpass) { $this->internal_ref_reset(); $bind[0]["value"] = $token; $bind[0]["type"] = "s";
		...
		if($f= $this->mysql->fetch_array($r)){
			...
			// Get User Data
			$x  = $this->get($f["fk_user"]);
			// Check if Blocked for Reset
			if(@$x["block_reset"] == 1) { $this->rec_request_code = 4; return 4; }		
			// Log Recovers
			...
			// Prepare Mail Bind
			$bindm[0]["type"] = "s"; $bindm[0]["value"] = $x["user_mail"];			
			// Delete the Shadow Mail from users which may have tried to Register that Mail
			if($this->mail_unique) { $this->mysql->query("UPDATE `".$this->dt_users."` SET user_shadow = NULL WHERE LOWER(user_shadow) = ?", $bindm); }
			// Change User Password
	...
	public function mail_edit($id, $newmail, $nointervall = false) { 
		$this->internal_ref_reset(); $bind[0]["type"] = "s"; $bind[0]["value"] = trim(strtolower($newmail) ?? '');	
		// Return if user not numeric
			...
			if($f= $this->mysql->fetch_array($r)){
				...
				if($xf=$this->mysql->fetch_array($x)) {
					...
					if(!$this->mail_unique) {
						...
						$this->mysql->query("UPDATE `".$this->dt_users."` SET user_shadow = NULL WHERE LOWER(user_shadow) = ?", $bindm);
					}
				}
				// All Okay
				$this->mc_request_code = 1; return 1;

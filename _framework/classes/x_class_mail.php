	...
	/*	__________ ____ ___  ___________________.___  _________ ___ ___  
		\______   \    |   \/  _____/\_   _____/|   |/   _____//   |   \ 
		 ...
		 |______  /______/  \______  /\___  /   |___/_______  /\___|_  / 
				\/                 \/     \/                \/       \/  x_class_mail Class needs phpMailer! (x_class_mail_smtp) */	
	class x_class_mail {
		...
		function __construct($host, $port = 25, $auth_type = false, $user = false, $pass = false, $from_mail = false, $from_name = false) {
			...
			$this->üassword 	 = $pass; // The Password fot Auth
			$this->smtpsecure 	 = $auth_type; // "tls" or "ssl"
			$this->port 		 = $port;  // The Port of Server example: 25
			$this->addReplyToMail = $from_mail; 
			$this->addReplyToName  = $from_name; } 
		...
		private function log_execute($subject, $content, $receiver, $attachments, $cc, $bcc, $success, $debug_message, $settings) {		
			...
			$this->l_mysql->query("INSERT INTO ".$this->l_table."(receiver, bcc, cc, attach, subject, msgtext, success, debugmsg, section) VALUES(?, ?, ?,?,?,?, '".$success."', ?, '".$this->cursection."');", $b);
			return true;
		}
		...
		public function send($to, $toname, $title, $mailContent, $ishtml = false, $FOOTER = false, $HEADER = false, $attachments = false) {
			...
			if($mail_status = $tmp_mailer->send()){		
				...
				$this->log_execute($subject, $realcontent, $to, $attachments, array(), array(), true, $tmp_mailer->ErrorInfo, false);				
		...
		public function mail($subject, $content, $receiver, $cc, $bcc, $attachment, $settings = array()) {
			...
			if(!isset($settings["host"])) 			{$tmp_mailer->Host 			= $this->host; 			} else { $tmp_mailer->Host 			= $settings["host"];}
			if(!isset($settings["smtpauth"])) 		{$tmp_mailer->SMTPAuth 	    = $this->smtpauth; 		} else { $tmp_mailer->SMTPAuth 		= $settings["smtpauth"];}
			if(!isset($settings["username"])) 		{$tmp_mailer->Username 	    = $this->username;		} else { $tmp_mailer->Username 		= $settings["username"];}
			if(!isset($settings["password"])) 		{$tmp_mailer->Password 	    = $this->password;  	} else { $tmp_mailer->Password 		= $settings["password"];}		
			if(!isset($settings["smtpsecure"])) 	{$tmp_mailer->SMTPSecure    = $this->smtpsecure; 	} else { $tmp_mailer->SMTPSecure 	= $settings["smtpsecure"]; }
			if(!isset($settings["port"])) 			{$tmp_mailer->Port 		    = $this->port;	 		} else { $tmp_mailer->Port 			= $settings["port"]; }
			if(!isset($settings["keep_alive"])) 	{$tmp_mailer->SMTPKeepAlive = $this->keep_alive; 	} else { $tmp_mailer->SMTPKeepAlive = $settings["keep_alive"]; }		
			if(!isset($settings["charset"])) 		{$tmp_mailer->CharSet 	   	= $this->charset; 		} else { $tmp_mailer->CharSet 		= $settings["charset"]; }
			if(!isset($settings["encoding"])) 		{$tmp_mailer->Encoding 		= $this->encoding; 		} else { $tmp_mailer->Encoding 		= $settings["encoding"]; }		
			if(!isset($settings["html"])) 			{$tmp_mailer->isHTML($this->html); 					} else { $tmp_mailer->isHTML($settings["html"]); }		
			if(!isset($settings["smtpdebuglevel"])) 		{$tmp_mailer->SMTPDebug     = $this->smtpdebuglevel; 	} else { $tmp_mailer->SMTPDebug 	= $settings["smtpdebuglevel"]; }
			if(!isset($settings["allow_insecure_connection"])) {
				if($this->allow_insecure_connection) {$tmp_mailer->SMTPOptions = ['ssl' => ['verify_peer' => false,'verify_peer_name' => false,'allow_self_signed' => true]];}
			} else { if($settings["allow_insecure_connection"]) {$tmp_mailer->SMTPOptions = ['ssl' => ['verify_peer' => false,'verify_peer_name' => false,'allow_self_signed' => true]];}  }		

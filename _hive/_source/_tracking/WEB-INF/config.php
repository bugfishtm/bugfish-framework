...
if(_BTM_SITE_MODE_ != "btm") { Header("Location: "._BTM_INTERNAL_MAINPAGEURL_."/_tracking"); exit(); }
if(!$object["user"]->loggedIn) { btm_error("You are not logged in!");exit();}
if($object["user"]->user_rank !== 1 AND $object["user"]->user_rank !== 2) { btm_error("You have no permission to view this page!"); exit(); }
...
//
define('DSN', 'mysqli://'._EXT_TRACKING_.'?charset=utf8mb4');
...
//
define('FORUM_LINK', 'https://'._BTM_MAIN_URL_.'/forum');
define('HELP_LINK', 'https://'._BTM_MAIN_URL_.'/wiki');


// Default sender for mail.
//
define('SENDER', 'Bugfish Time Tracker <system@bugfish.eu>');

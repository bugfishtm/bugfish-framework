					...
					if(!$this->only200 OR ($this->only200 AND http_response_code() == 200)) {
						...
						if(@$_SESSION[$this->precookie."x_class_hitcounter"] != "ok") { 
							...
							$sresult = mysqli_fetch_array($this->mysql->query("SELECT * FROM `".$this->mysqltable."` WHERE full_url = ?;", $bindar), MYSQLI_BOTH);
								...
								} else { $this->mysql->query("UPDATE ".$this->mysqltable." SET arrivals = arrivals + 1 WHERE full_url = ?;", $bindar);}
							$_SESSION[$this->precookie."x_class_hitcounter"] = "ok";}
							
						$ishittedarray = false;
						if(!is_array($_SESSION[$this->precookie."x_class_hitcounter_s"])) { $_SESSION[$this->precookie."x_class_hitcounter_s"] = array(); }
						if(is_array($_SESSION[$this->precookie."x_class_hitcounter_s"])) {
							foreach($_SESSION[$this->precookie."x_class_hitcounter_s"] as $key => $value) {
						...
						if(!$ishittedarray AND !$isarrival AND !$this->onlyarrivals) { 
							...
							else {$this->mysql->query("UPDATE ".$this->mysqltable." SET switches = switches + 1 WHERE full_url = ?;", $bindar);}
							array_push($_SESSION[$this->precookie."x_class_hitcounter_s"], $this->urlmd5);
						}				
					}
				}
			}

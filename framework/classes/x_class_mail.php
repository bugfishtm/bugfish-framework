...
class PHPMailer
{
	...
	class x_class_mail_exception extends \Exception
	{
	...
	class x_class_mail {
		...
		private $defheader 		= ""; 	private $deffooter = ""; public function change_default_template($header, $footer) {$this->defheader = $header;$this->deffooter = $footer;} 
		...
		public function send($to, $toname, $title, $mailContent, $ishtml = false, $FOOTER = false, $HEADER = false, $attachments = false) {
			...
			$xFOOTER = "";
			$xHEADER = "";
			if(!$FOOTER) {$xFOOTER = $this->deffooter;}
			if(!$HEADER) {$xHEADER = $this->defheader;}
